...
- [ ] **One `convex/model/analytics.ts` producer** deriving per-event
      ...
      with truncation flags, never stored.
- [ ] **One analytics panel** on the event control center (collapsible,
      ...
      KPI grid (the review's objection stands).
- [ ] Convex tests over seeded audit fixtures, incl. the empty-event case
      and negative authz.
- *Mobile*: the panel is a stacked list of sentences; no chart is required
  for v1 — numbers with populations beat sparklines this week.
