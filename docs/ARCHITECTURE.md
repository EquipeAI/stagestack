...
**Why the same package is declared in two workspaces** (audited C4; the duplication is load-bearing, do not "clean it up"). Root `workspaces` is `apps/*`, so the repo root is not a workspace member — and Vercel installs ONLY the `apps/web` subtree. Anything `convex/` types pull in must therefore be declared in `apps/web` too or the build loses it (`convex-helpers`, `@convex-dev/resend`, `@convex-dev/rate-limiter`, `sharp`, `convex`), which is the fix for the Aug-9 TS2307 cascade. `vitest` is in both because each workspace runs its own suite. The remaining overlaps are deliberate divergences, not drift: `typescript` is the TS6 alias in `apps/web` (whose `typecheck` script resolves `@typescript/native` explicitly) and plain `^5.9.0` in the worker; `@types/node` and `xlsx` are genuine needs of two different runtimes. Nothing is hoistable. `npm dedupe` was also run and its result REJECTED: its only real win was two nested `safe-buffer` copies, paid for by floating `@smithy/*` and `globals` onto transitive versions this tree has never been tested against — the lockfile is the pin, so a dedupe that rewrites resolved versions is a version bump wearing a hygiene costume.
...
- Saved views = table state serialized into typed/validated URL search params (`validateSearch`) — shareable URLs for free. Since W2 the params vocabulary of every module table lives ONCE in `convex/shared/viewParams.ts`: the routes' `validateSearch` re-exports it and the `savedViews` capability validates stored params with the same function, so a stored view can never carry a param the route would drop. Naming a view is `savedViews` (event-scoped, per-user, one optional default per module); sharing one is still just the URL — code-level presets (e.g. the rail's Decisions entry) live beside the parsers rather than in any route.
