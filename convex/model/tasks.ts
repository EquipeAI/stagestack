...
/* One event's worth of rows — the shared ceilings imported from
 * `lib/readCaps` above. Bounded reads only; the organizer console paginates
 * long before an event reaches those numbers. */
...
// Moved verbatim to `./taskFiles` (it reads task uploads and headshot
// snapshots and writes nothing, so it depended on none of the rules above).
// Re-exported here because both this module's callers and the public wrappers
// in `convex/tasks.ts` reach it through `Tasks.*`.
export {
  completeHeadshotContactPage,
  exportBundle,
  filesLibrary,
  legacyLibraryFileRows,
} from "./taskFiles";
export type {
  BundleFile,
